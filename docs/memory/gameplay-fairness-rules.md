...
- **Remaining (future animation polish, not done — do not risk the finish state machine):** the hero still *slides* horizontally to `FINISH_HERO_REACH_X` to meet the note. A more natural feel would be to move/absorb the **note into the hero** at the hero's running position instead of sliding the hero to a fixed point. This is a finish-flow rework (`updateFinishObjects` + the reach constants) and should be a dedicated slice with its own validation.
